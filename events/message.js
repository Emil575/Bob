    ...
    try {
        ...
        function setlevel() {
            try {
                ...
                console.log("RANKING:".underline.red + " :: " + error.stack.toString().red)
                message.reply({content: "PLEASE ADD A RANKUSER!"});
            }
        }
        ...
        function removelevel() {
            try {
                 ...
                 * @info you can tag him
                 */
                if (!args[0]) return message.reply({content: "PLEASE ADD A RANKUSER!"});
                let rankuser = message.mentions.users.first();
                if (!rankuser) return message.reply({content: "PLEASE ADD A RANKUSER!"});
                // if(rankuser.bot) return message.reply("NO BOTS!");
                ...
                const embed = new Discord.MessageEmbed()
                    ...
                    .setColor(embedcolor);
                message.channel.send({embeds: [embed]}).catch(e=>console.log("ranking: " + e));
                rank(rankuser); //also sending the rankcard
                const sssembed = new Discord.MessageEmbed()
                .setColor(embedcolor)
        ...
        function levelinghelp() {
            const embed = new Discord.MessageEmbed()
                ...
                .setTimestamp()
                .setDescription(`> **HELP:**  \`!levelinghelp\``)
                .setColor(embedcolor)
                .addFields([{
                        name: "`rank [@User]`",
