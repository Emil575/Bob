    ...
    async function first_layer() {
      ...
      async function menuselection(menu) {
        ...
        if (menu.values[0] == "Information") {
          ...
          await message.channel.send("<a:yes:833101995723194437> **The Current Anti-Nuke Settings**", {
                ...
                .then(async collected => {
                  ...
                  return message.reply({
                    embed: new Discord.MessageEmbed()
                      ...
                      .addField("__**Current Whitelisted Users:**__", `>>> ${client.Anti_Nuke_System.get(interaction.guild.id, "antideleteuser.whitelisted.users").length == 0 ? "**No One**" : client.Anti_Nuke_System.get(interaction.guild.id, "antideleteuser.whitelisted.users").length > 15 ? client.Anti_Nuke_System.get(message.guild.id, "antideleteuser.whitelisted.users").slice(0, 15).map(x=>`<@${x}>`).join(", ") + ` ***and ${client.Anti_Nuke_System.get(message.guild.id, "antideleteuser.whitelisted.users").length - 15} more...***`: client.Anti_Nuke_System.get(message.guild.id, "antideleteuser.whitelisted.users").map(x=>`<@${x}>`).join(", ")}`)
                      .addField("__**Current Whitelisted Roles:**__", `>>> ${client.Anti_Nuke_System.get(interaction.guild.id, "antideleteuser.whitelisted.roles").length == 0 ? "**No One**" : client.Anti_Nuke_System.get(message.guild.id, "antideleteuser.whitelisted.roles").length > 15 ? client.Anti_Nuke_System.get(message.guild.id, "antideleteuser.whitelisted.roles").slice(0, 15).map(x=>`<@&${x}>`).join(", ") + ` ***and ${client.Anti_Nuke_System.get(message.guild.id, "antideleteuser.whitelisted.roles").length - 15} more...***`: client.Anti_Nuke_System.get(message.guild.id, "antideleteuser.whitelisted.roles").map(x=>`<@&${x}>`).join(", ")}`)
                      .setFooter(es.footertext, es.footericon)
                  });
                })
                .catch(e => {
