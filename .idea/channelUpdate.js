 ...
 try {
  ...
  if(log) {
   ...
   oldChannel.guild.fetchAuditLogs().then(logs => {
    ...
    if(oldChannel.Permission !== newChannel.Permission) {
     const newpermission = new Discord.MessageEmbed()
       .setTitle("chaneel permission changed")
       .setThumbnail(uavatar)
       .addField("old channel permission", `\`\`\`\ ${oldChannel.Permission || "(not set)"} \`\`\` `)
       .addField("new channel permission", `\`\`\`\ ${newChannel.Permission || "(not set)"} \`\`\` `)
       .addField("channel id", `${newChannel.id}`)
       .addField("channel type", `<@${userid}> (ID: ${userid})`)
       .addField("created at", `${oldChannel.createdAt}`)
       .addColor("RANDOM")
       .setFooter(oldChannel.guild.name , oldChannel.guild.iconURL())
     log.send(newpermission);
    }
