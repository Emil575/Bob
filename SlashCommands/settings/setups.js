...
if(SUB_CMD === 'ticket'){
  ...
  interaction.channel.send({
    ...
    msg.channel.awaitMessages({
      ...
      switch (collected.first().content.toString()) {
        ...
        case "3":
          ...
          interaction.channel.send({
            ...
            msg.channel.awaitMessages({
              ...
              if (!role) interaction.channel.send({
                content: "CANCELLED, u didn't Pingged a valid Role"
              })
              console.log(role)
              interaction.channel.send({
                content: "Successfully **added**: `" + role.name + "` to the Admin-Roles"
              });
          ...
          interaction.channel.send({
            ...
            msg.channel.awaitMessages({
              ...
              try {
                ...
                interaction.reply({
                  content: "Successfully **removed**: `" + role.name + "` from the Admin-Roles"
                });
              } catch {
                interaction.reply({
                  content: "ERROR -> Resetted all Admin roles"
                })
        ...
        case "5":
          if (!client.setups.get(interaction.guild.id, "ticketsystem.enabled")) {
            client.setups.set(interaction.guild.id, true, "ticketsystem.enabled");
            interaction.channel.send({
              content: "Succesfully enabled the Ticket-System"
            })
            ...
            interaction.channel.send({
              content: "Succesfully disable the Ticket-System"
            })
          } else {
            interaction.reply({
      ...
      console.log(error)
      return interaction.reply({content: "SORRY BUT YOUR TIME RAN OUT!"})
    })
  })
}
    },
